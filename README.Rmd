...
- [2.3 Solutions for Violations of Model Diagnostics](#23-solutions-for-violations-of-model-diagnostics)
...
- [4.1 _Post Hoc_ Power Analysis](#41-post-hoc-power-analysis)
- [4.2 _A Priori_ Power Analysis](#42-a-priori-power-analysis)
...
```{r exp_model, fig.width=12, fig.height=8}
lmm_ex <- lmmModel(
  data = grwth_data,
  grwth_model = "exp",
  sample_id = "subject",
...
knitr::include_graphics("vignettes/Gompertz_model.png")
```
...
```{r ranefDiagnostics, fig.width=10, fig.height=10}
...
```{r residDiagnostics, fig.width=10, fig.height=14}
...
```{r exp_model_varIdent, fig.width=12, fig.height=8}
lmm_ex_var <- lmmModel(
  data = grwth_data,
  sample_id = "subject",
  time = "Time",
...
```{r ObsvsPred, fig.width=10, fig.height=10}
...
```{r CookDistance_fixef, eval=FALSE}
CookDistance(lmm_ex_var, type = "fixef")
```

```{r, echo=FALSE,  fig.width=10, fig.height=8}
knitr::include_graphics("vignettes/CooksDistance_fixef.png")
```


**Cook's Distances based on the change of fitted values**

```{r CookDistance_fitted, eval=FALSE}
CookDistance(lmm_ex_var, type = "fitted")
```

```{r, echo=FALSE,  fig.width=10, fig.height=8}
knitr::include_graphics("vignettes/CooksDistance_fitted.png")
```
...
```{r logLikSubjectDisplacements, eval=FALSE}
logLikSubjectDisplacements(lmm_ex_var, var_name = "SampleID")
```

```{r, echo=FALSE,  fig.width=10, fig.height=8}
knitr::include_graphics("vignettes/loglikDisp.png")
```
...
```{r bliss_robust, fig.width=12, fig.height=10}
...
```{r hsa, fig.width=12, fig.height=10}
...
```{r ra, fig.width=12, fig.height=10}
...
```{r apriori_pwr_size, fig.width=10, fig.height=8}
...
```{r apriori_pwr_time, fig.width=10, fig.height=8}
PwrTime(npg = npg,
        time = max_time,
        type = "max",
        grwrControl = round(estimates$Control,3),
...
```{r apriori_pwr_freq, fig.width=10, fig.height=8}
PwrTime(npg = npg,
        time = freq_time,
        type = "freq",
        grwrControl = round(estimates$Control,3),
...
```{r apriori_pwr_var, fig.width=14, fig.height=8}
