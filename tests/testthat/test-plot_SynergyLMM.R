...
test_that("plot_lmmSynergy runs without error on valid input", {
  ...
  # 'ObsvsPred' returns a lattice object whose length depends on lattice
  # internals and on the fitted model, so only assert that a plot is returned.
  obs_pred <- plot_SynergyLMM(lmm, "ObsvsPred")
  expect_s3_class(obs_pred, "trellis")
  expect_gt(length(obs_pred), 0)
})
