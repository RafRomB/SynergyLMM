...
However, as it can be seen, sometimes this can result in some errors, due to the impossibility fit the model at the earlier time points (due to insuficient data). In these cases, and easy solution is to simply increase the value of the `min_time` argument, to increase the minimum time for which to start calculating the synergy:
